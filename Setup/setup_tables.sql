...
-- Schema mydb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `cuteqian_db` DEFAULT CHARACTER SET utf8 ;
USE `cuteqian_db` ;

-- -----------------------------------------------------
-- Table `cuteqian_db`.`Author`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Author` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Author` (
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Editor`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Editor` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Editor` (
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`RICodes`
-- -----------------------------------------------------

DROP TABLE  IF EXISTS `cuteqian_db`.`RICodes` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`RICodes` (
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Manuscript`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Manuscript` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Manuscript` (
  ...
  CONSTRAINT `fk_Manuscripts_Authors`
    ...
    REFERENCES `cuteqian_db`.`Author` (`idAuthor`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Manuscripts_Editor`
    FOREIGN KEY (`idEditor`)
    REFERENCES `cuteqian_db`.`Editor` (`idEditor`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Manuscripts_RICodes`
    FOREIGN KEY (`code`)
    REFERENCES `cuteqian_db`.`RICodes` (`code`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Reviewer`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Reviewer` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Reviewer` (
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Feedback`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Feedback` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Feedback` (
...
DELIMITER //
CREATE TRIGGER feedbakc_score BEFORE INSERT ON `cuteqian_db`.`Feedback`
       FOR EACH ROW
       BEGIN
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Assignment`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Assignment` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Assignment` (
  ...
  CONSTRAINT `fk_Manuscripts_has_Reviewers_Manuscripts`
    ...
    REFERENCES `cuteqian_db`.`Manuscript` (`idManuscript`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Manuscripts_has_Reviewers_Reviewers`
    FOREIGN KEY (`idReviewer`)
    REFERENCES `cuteqian_db`.`Reviewer` (`idReviewer`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Assignments_Feedbacks`
    FOREIGN KEY (`idFeedback`)
    REFERENCES `cuteqian_db`.`Feedback` (`idFeedback`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Issue`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Issue` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Issue` (
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`Typesetting`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `cuteqian_db`.`Typesetting` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`Typesetting` (
  `idManuscript` INT NOT NULL,
  `publicationYear` INT NOT NULL,
  `publicationPeriod` INT NOT NULL,
  ...
  CONSTRAINT `fk_Typesettings_Issues`
    ...
    REFERENCES `cuteqian_db`.`Issue` (`publicationYear` , `publicationPeriod`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
DELIMITER //
CREATE TRIGGER typesetting_rules BEFORE INSERT ON `cuteqian_db`.`Typesetting`
       FOR EACH ROW
       BEGIN
           IF ((SELECT `pages` FROM `cuteqian_db`.`Issue` 
            WHERE `publicationYear` = NEW.publicationYear AND `publicationPeriod` = NEW.publicationPeriod) + 1 != NEW.beginPage
            OR (SELECT `typesetPages` FROM `cuteqian_db`.`Manuscript` 
           ...
           ELSE
              UPDATE `cuteqian_db`.`Issue` SET `pages` = (SELECT `typesetPages` FROM `cuteqian_db`.`Manuscript` 
...
-- -----------------------------------------------------
-- Table `cuteqian_db`.`InterestList`
-- -----------------------------------------------------

DROP TABLE IF EXISTS `cuteqian_db`.`InterestList` CASCADE;

CREATE TABLE IF NOT EXISTS `cuteqian_db`.`InterestList` (
  `code` INT NOT NULL,
  `idReviewer` INT NOT NULL,
...
CREATE TRIGGER valid_publication_period BEFORE INSERT ON `cuteqian_db`.`Issue`
 FOR EACH ROW
    BEGIN
...
CREATE TRIGGER max_number_interests BEFORE INSERT ON `cuteqian_db`.`InterestList`
       FOR EACH ROW
       BEGIN
           IF ((SELECT COUNT(*) FROM `cuteqian_db`.`InterestList` `re` WHERE `re`.`idReviewer` = NEW.idReviewer) >= 3)
           THEN
              SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'You only can enter max three number of interests';
           END IF;
       END;//

CREATE TRIGGER valid_manuscript BEFORE INSERT ON `cuteqian_db`.`Manuscript`
       FOR EACH ROW
       BEGIN
           IF ((SELECT COUNT(*) FROM `cuteqian_db`.`InterestList` `re` WHERE `re`.`code` = NEW.code) <= 0)
           THEN
              SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Your paper cannot be considered at this time';
           END IF;
       END;//

CREATE TRIGGER reset_submitted_status BEFORE DELETE ON `cuteqian_db`.`Reviewer`
       FOR EACH ROW
       BEGIN
           UPDATE `cuteqian_db`.`Manuscript` `ma` SET `ma`.`status` = 'Submitted' 
